...
from subgraph_match.filters import GuardContext, NogoodGuard, ReservationGuard, SearchGuard
...
class GUPMatcher:
    """A correctness-first GUP-lite matcher with pluggable guards."""

    name = 'gup_lite'

    def __init__(self, config: GUPConfig | None = None) -> None:
    ...
    def _search(
        ...
        for data_vertex in candidates.get(query_vertex, []):
            ...
            before_results = statistics.result_mappings
            self._search(
                depth=depth + 1,
                query_graph=query_graph,
                data_graph=data_graph,
