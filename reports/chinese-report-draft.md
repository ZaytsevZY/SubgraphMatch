...
> 标注说明：为便于对照课程作业要求，本文在相关章节标题中加入“【对应打分点】”标记。其中：
> - 选题背景简介：提交要求 5.(a)
> - 文献调研报告：选题 2 第 1 项，15 分；对应提交要求 5.(b)
> - 复现论文的算法思路：选题 2 第 2 项，20 分；对应提交要求 5.(c)
> - 多数据集实验与结果分析：选题 2 第 3 项，20 分；对应提交要求 5.(d)
> - 过滤策略消融分析：选题 2 第 4 项，15 分；对应提交要求 5.(d)
> - 选做：结果复用：选题 2 第 5 项（选做）
> - 总结、发展历史与个人心得：提交要求 5.(f)
> - 组内分工说明：提交要求 5.(g)

---

## 1. 引言（Introduction）【对应：提交要求 5.(a) 选题背景简介】
...
## 2. 文献调研与多维分类体系（Literature Review and Multi-dimensional Taxonomy）【对应：选题 2 第 1 项，15 分；提交要求 5.(b)】
...
## 3. 问题定义与评测指标（Problem Definition and Evaluation Metrics）【支撑：选题 2 第 2/3/4 项】
...
## 4. 复现论文与复现范围（Reproduced Paper and Reproduction Scope）【对应：选题 2 第 2 项，20 分；提交要求 5.(c)】
...
### 4.1 GuP 的两个核心机制【对应：选题 2 第 2 项核心算法思想】
...
### 4.2 复现范围（Reproduction Scope）【对应：选题 2 第 2 项复现说明】
...
## 5. 实现细节（Implementation Details）【支撑：选题 2 第 2/3/4 项】
...
## 6. 实验设置（Experimental Setup）【支撑：选题 2 第 3/4 项】
...
### 6.1 使用的数据集【对应：选题 2 第 3 项的数据集要求】
...
### 6.3 四种比较配置【对应：选题 2 第 4 项的策略开关设计】
...
## 7. 多数据集实验结果（Required Experimental Results on Multiple Datasets）【对应：选题 2 第 3 项，20 分；提交要求 5.(d)】
...
### 7.1 三个成功数据集【对应：选题 2 第 3 项“3–4 组数据集”】
...
### 7.5 Boundary datasets【补充说明：边界 workload，不单独计分但增强分析完整性】
...
## 8. 过滤策略消融分析（Ablation Study of Filtering Strategies）【对应：选题 2 第 4 项，15 分；提交要求 5.(d)】
...
## 9. 选做项：子图匹配结果复用（Optional Extension: Result Reuse for Related Queries）【对应：选题 2 第 5 项（选做）】

除必做部分外，我们还实现了一个较小但完整的选做扩展：讨论后续查询能否复用先前查询的结果，并据此设计一个简单可验证的优化算法。当前扩展聚焦于一个受限但很自然的场景：数据图保持不变，新的查询图 $Q_{next}$ 由旧查询图 $Q_{prev}$ **增加一条查询边**得到，同时顶点集合和所有顶点标签保持不变。

### 9.1 受限场景与复用规则

这个设定对应一种常见的查询细化流程：用户先运行一个相对宽松的模式，然后再加入一条缺失的结构约束，把查询收紧。在这种情况下，$Q_{next}$ 的任意合法结果映射一定已经属于 $Q_{prev}$ 的结果映射。因此，后续查询不必完全从头再跑一次子图匹配，而可以直接扫描已经物化的 $Q_{prev}$ 结果集，只检查新加入的那条查询边在数据图中是否成立。

形式化地说，如果 $Q_{next}$ 是在 $Q_{prev}$ 上新增一条查询边 $(u,v)$ 得到的，则有：

\[
\mathrm{Result}(Q_{next}) = \{ f \in \mathrm{Result}(Q_{prev}) \mid (f(u), f(v)) \in E_G \}.
\]

因此，这个选做项把“先前查询的结果映射”视为可复用状态，并比较两种成本：

- **reuse cost**：只在旧结果集上做新增边约束检查的增量代价；
- **scratch cost**：对新查询 $Q_{next}$ 从头执行完整子图匹配的代价。

这里默认前一个查询已经执行完成，并且其 `result mappings` 已经被保存，因此比较重点是“下一条相关查询的增量复用成本”，而不是把两次查询的总成本混在一起。

### 9.2 选做实验结果

英文报告中的结果复用实验包含 1 组 toy query pair 和 2 组真实 query pair。三组实验都满足同一个结论：**reuse results 与 scratch results 完全一致**，因此在当前受限设定下，复用规则是正确的。

具体结果如下：

- **Toy**：上一条查询有 `2` 个结果，复用后仍为 `2`，与从头计算一致；复用耗时约 `0.00075 ms`，从头计算约 `0.00546 ms`。
- **HPRD**：`d4-10-minus-e01 -> d4-10` 这组查询中，上一条查询已有 `4` 个结果；新增边不会过滤任何结果，因此复用后仍为 `4`，与从头计算一致；复用耗时约 `0.00250 ms`，而从头计算约 `2.68837 ms`。
- **Human**：`d4-1-minus-e01 -> d4-1` 这组查询中，上一条查询共有 `10384` 个结果；新增边过滤掉其中 `72` 个，最终保留 `10312` 个结果，与从头计算的结果完全一致；复用耗时约 `1.60363 ms`，从头计算约 `66.45746 ms`。

### 9.3 结果分析

这些结果支持三个观察。

第一，**正确性得到了直接验证**。在三组 query pair 上，复用结果和从头计算结果完全相同，说明“上一查询结果集 + 新增边过滤”在当前设定下是一个 sound 的复用规则。

第二，**即使新增边没有改变最终结果数量，复用仍然可能显著更便宜**。HPRD 就是最典型的例子：虽然前后查询的最终结果数都为 `4`，但从头计算仍然需要重新构造和执行完整搜索过程，而复用只需在已有结果上追加一次很轻的过滤检查。

第三，**Human query pair 展示了最明显的 workload-level 收益**。宽松查询先得到 `10384` 个结果，新增边后只需过滤掉 `72` 个，就得到与从头匹配完全相同的 `10312` 个结果；而增量复用耗时只有约 `1.60 ms`，远低于从头计算的 `66.46 ms`。这说明对于同一数据图上的交互式查询细化场景，结果复用确实可能带来很明显的收益。

### 9.4 局限性

当前选做项的局限也很明确：

- 只支持“**新增一条查询边**”这一种非常受限的后续查询关系；
- 还不支持顶点增加、任意查询改写，或更一般的查询编辑；
- 复用的是最终 `result mappings`，而不是更细粒度的 `partial mappings`、nogood information 或搜索中间状态。

因此，这个选做项还不是一个通用的结果复用框架，但作为课程项目中的额外扩展，它已经完成了一个**正确、可解释、且实验现象明确**的 workload-level 优化示例，也正面回应了作业中“后续查询能否利用先前查询结果”的问题。

---

## 10. 总结、发展背景与心得（Conclusion, Development Context, and Reflection）【对应：提交要求 5.(f)】
...
## 11. 组内分工（Division of Labor）【对应：提交要求 5.(g)】
