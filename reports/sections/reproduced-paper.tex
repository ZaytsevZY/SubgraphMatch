...
Table~\ref{tab:repro-scope} summarizes which original GuP components were reproduced in this project.

\begin{table*}[t]
  \caption{Reproduction scope of the course-project GuP implementation.}
  \label{tab:repro-scope}
  \centering
  \small
  \begin{tabular}{p{0.20\textwidth}p{0.10\textwidth}p{0.28\textwidth}p{0.12\textwidth}p{0.20\textwidth}}
    \toprule
    Original GuP component & Implemented & Our design & Faithfulness & Expected impact on results \\
    \midrule
    ...
    Matching order & Yes & Simple order by candidate size, degree, and id & Partial & May reduce pruning strength on harder workloads \\
    ...
    Guarded candidate-space engineering & Partial & Only the minimum structures required by the prototype & Partial & Limits peak performance and scalability \\
...
The most important boundary is therefore not correctness but \emph{faithfulness of engineering detail}. Our reproduction is sufficient for course-project analysis of the main algorithmic ideas and for ablation of the two guard mechanisms, but it should not be interpreted as a claim of matching the original production-level performance of GuP on every workload.
