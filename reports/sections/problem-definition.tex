...
In our implementation, all graphs are undirected, vertex-labeled, and loop-free. We do not consider edge labels, subgraph homomorphism, or dynamic graph updates in the mandatory part of the project.
...
The last metric deserves special clarification. Our internal counter \texttt{pruned\_partial\_mappings} records \emph{all} rejected extensions, including injectivity and edge-consistency conflicts. However, the assignment specifically asks for the number of partial mappings filtered out by the paper's own strategies. Therefore, in the report we separately extract:
\begin{itemize}[nosep]
...
This distinction is important for grading: the total prune count reflects the whole feasibility-check framework, while the paper-filtered count reflects the contribution of the reproduced GuP-style filtering strategies themselves.
