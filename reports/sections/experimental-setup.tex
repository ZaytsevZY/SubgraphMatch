...
The benchmark data were taken from the public dataset package used in the GuP study and loaded through the project's unified \texttt{.graph} parser. For clarity, we explicitly separate \emph{main successful datasets} from \emph{time-boxed boundary datasets}.
...
For every successful run, we record runtime, result mappings, partial mappings, and paper-filtered partial mappings. The paper-filtered count is computed from the prune-reason log and currently consists of the number of extensions rejected by the reservation and nogood guards. Because the current implementation rarely activates useful nogood pruning on the main workloads, the paper-filtered count is dominated by the reservation guard in the final results.

For boundary datasets, we use a 120-second time box. If a run does not finish within this limit, it is explicitly reported as \texttt{timeout} instead of being silently excluded. This makes the scalability boundary of the prototype explicit and reproducible.
